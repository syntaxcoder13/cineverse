...
import { exploreTV } from '../utils/tmdb';
...
const TVShows = () => {
    ...
    const [selectedShow, setSelectedShow] = useState({ id: null, type: 'tv' });

    const dispatch = useDispatch();
    const { isAuthenticated } = useSelector((state) => state.auth);

    const fetchShows = async (pageNum, currentGenre, currentSort) => {
        setLoading(true);
        try {
            const data = await exploreTV(currentGenre, currentSort, pageNum);
            if (pageNum === 1) {
